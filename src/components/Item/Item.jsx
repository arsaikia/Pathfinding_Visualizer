...
const Item = ({ ridx, cidx }) => {
  ...
  const getColor = () => {
    ...
    if (ridx === end.current.x && cidx === end.current.y) {
      return FIXED_COLOR;
    }
  ...
  const getBackground = () => {
    if (ridx === begin.current.x && cidx === begin.current.y)
      return 'url(' + require('../../Icons/guitar.svg') + ')';
    if (ridx === end.current.x && cidx === end.current.y)
      return 'url(' + require('../../Icons/destination.svg') + ')';
    if (type === ITEM_SHORTEST) {
      return 'url(' + require('../../Icons/paperPlane.svg') + ')';
    }
    if (type === ITEM_CLICKED) {
      return 'url(' + require('../../Icons/brickWall.svg') + ')';
    }
    return 'url(' + require('../../Icons/square.svg') + ')';
  };
